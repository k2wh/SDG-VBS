  ...
  return (
    <div>
      ...
      <div className="mb-4 flex justify-end">
        <button
          onClick={() => { setEditId(null); setForm({ ...emptyForm }); setStakeholdersSelecionados([]); setProjecao([]); setShowForm(true); }}
      ...
      <FormModal open={showForm} onClose={resetForm} title={editId ? 'Editar Propagação' : 'Nova Propagação'} maxWidth="max-w-6xl">
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-gray-50 rounded-lg border border-gray-200 p-5">
              ...
              <div className="space-y-4">
                <FormField label="Benefício" type="select" value={form.beneficio_id} onChange={(v) => set('beneficio_id', v)} options={beneficiosOpts} required searchable />
                <div>
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    <div className="flex-1">
                      <FormField label="" type="select" value={stakeholderSelect} onChange={setStakeholderSelect} options={shOpts} searchable />
                    </div>
                    <button type="button" onClick={addStakeholder} className="bg-primary-600 hover:bg-primary-700 text-white rounded-lg px-3 py-2 text-sm font-medium transition-colors shrink-0 self-end">
                      Vincular
                    </button>
                  </div>
                  {stakeholdersSelecionados.length > 0 && (
                    <div className="flex flex-wrap gap-2 mt-3">
                      {stakeholdersSelecionados.map(sh => (
                        <span key={sh.id} className="inline-flex items-center gap-1 bg-primary-50 text-primary-700 text-xs font-medium px-2.5 py-1 rounded-full border border-primary-200">
                          {sh.nome}
                          <button type="button" onClick={() => removeStakeholder(sh.id)} className="text-primary-400 hover:text-primary-700 transition-colors ml-0.5">
                ...
                <FormField label="Registrar como se dá a propagação" type="textarea" value={form.registro_propagacao} onChange={(v) => set('registro_propagacao', v.slice(0, 1000))} rows={3} placeholder="Descreva como a propagação ocorrerá..." />
                <p className="text-xs text-gray-400 -mt-2 text-right">{(form.registro_propagacao || '').length}/1000</p>
                <FormField label="Riscos de não propagação" type="textarea" value={form.riscos_nao_propagacao} onChange={(v) => set('riscos_nao_propagacao', v.slice(0, 700))} rows={2} placeholder="Quais riscos se a propagação não ocorrer?" />
                <p className="text-xs text-gray-400 -mt-2 text-right">{(form.riscos_nao_propagacao || '').length}/700</p>
                <FormField label="Possíveis efeitos colaterais" type="textarea" value={form.efeitos_colaterais} onChange={(v) => set('efeitos_colaterais', v.slice(0, 700))} rows={2} placeholder="Efeitos colaterais possíveis da propagação..." />
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="number" min="0" max="9999999" value={form.stakeholders_estimados} onChange={(e) => set('stakeholders_estimados', e.target.value)}
                      className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all duration-150 hover:border-gray-400" placeholder="0" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Períodos</label>
                    <input type="number" min="1" max="36" value={form.periodos} onChange={(e) => {
                      const v = Math.min(36, Math.max(0, parseInt(e.target.value) || 0));
                      set('periodos', v || e.target.value);
                    }} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all duration-150 hover:border-gray-400" placeholder="1" />
                  </div>
                </div>
                <FormField label="Abordagem principal de propagação" type="select" value={form.abordagem} onChange={(v) => set('abordagem', v)} options={ABORDAGENS.map(a => ({ value: a.value, label: a.label }))} required />
                ...
                {form.abordagem === 'Outros' && (
                  <div>
                    ...
                    <input type="text" maxLength={80} value={form.justificativa_outros} onChange={(e) => set('justificativa_outros', e.target.value)}
                      className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all duration-150 hover:border-gray-400" placeholder="Descreva a abordagem..." />
                ...
                <div className="grid grid-cols-3 gap-3 mt-2">
                  ...
                  <div className={`border rounded-lg p-3 text-center ${coberturaFinal >= 100 ? 'bg-green-50 border-green-200' : coberturaFinal > 0 ? 'bg-blue-50 border-blue-200' : 'bg-white border-gray-200'}`}>
                    ...
                    {projecao.map((row, i) => {
                      ...
                      return (
                        <tr key={i} className="hover:bg-white transition-colors">
                          ...
                          <td className="px-4 py-2">
                            <input type="number" min="0" value={row.beneficiarios_previstos} onChange={(e) => setProjecaoRow(i, e.target.value)}
                              className="w-full max-w-xs border border-gray-300 rounded-lg px-3 py-1.5 text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all duration-150 hover:border-gray-400" placeholder="0" />
                          </td>
          ...
          <div className="flex justify-end gap-3">
            ...
            <button type="submit" className="bg-primary-600 hover:bg-primary-700 text-white rounded-lg px-5 py-2 text-sm font-medium transition-colors">
